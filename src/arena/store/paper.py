...
def apply_round_to_equity(
    ...
    *,
    ...
    signals. Each position's PnL is::
        ...
        stake_per_position * (r / 100) * dir_mult

    where ``r`` is the realized % move from ``price_at_signal`` to
    ``price_at_eval`` and ``dir_mult`` is +1 for LONG, -1 for SHORT. FLAT
    signals hold cash, so an all-FLAT round leaves equity unchanged. Signals
    with a non-positive ``price_at_signal`` (invalid market data) also hold
    cash rather than dividing by zero.
    """
    positions = [
